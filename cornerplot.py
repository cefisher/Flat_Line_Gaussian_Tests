...
def corner(xs, bins=20, range=None, weights=None, color="k",
    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights,
                              range=np.sort(range[i]), linewidth=3.5, **hist_kwargs)
        else:
        ...
        if truths is not None and truths[i] is not None:
            ax.axvline(truths[i], color=truth_color, linewidth=2.5)
            # ax.axvline(truths[i][0], color=truth_color, linestyle='-')
            # ax.axvline(truths[i][0] + truths[i][1], color=truth_color, linestyle=':')
            # ax.axvline(truths[i][0] - truths[i][2], color=truth_color, linestyle=':')
        ...
        for j, y in enumerate(xs):
            ...
            hist2d(y, x, ax=ax, range=[range[j], range[i]], weights=weights,
                   smooth=smooth, bins=[bins[j], bins[i]], linewidth=2.5,
                   **hist2d_kwargs)
            #
    ...
    #
    # pressure = tp_profile_samples[:, 0]
    # temp_samples = tp_profile_samples[:, 1:]
    # temp_min, temp_median, temp_max = np.quantile(temp_samples, [0.16, 0.5, 0.84], axis=1)
    # ax.fill_betweenx(pressure, temp_min, temp_max, color='#ffb4b4')
    # ax.plot(temp_median, pressure, color='#cf0000')
    # ax.set_yscale('log')
    # ax.invert_yaxis()
    #
    ...
    # ax.set_ylabel('Pressure (bar)', fontsize=label_size)
